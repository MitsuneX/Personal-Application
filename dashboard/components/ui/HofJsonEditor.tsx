...
import {
  ...
  type FieldDiff,
} from "@/lib/data/hofSchema";

interface HofJsonEditorProps {
  profile: Partial<HallOfFameEntry>;
  onApply: (updated: Partial<HallOfFameEntry>, mode: "replace" | "merge") => void;
}
...
export function HofJsonEditor({ profile, onApply }: HofJsonEditorProps) {
  ...
  const [parsed, setParsed] = useState<Partial<HallOfFameEntry> | null>(null);
  ...
  const handleValidate = useCallback(() => {
    ...
    const result = validateHofJson(raw);
    if (!result.valid) {
      setValidationState("invalid");
      ...
      setErrors([]);
      const norm = raw as Partial<HallOfFameEntry>;
      setParsed(norm);
      setDiffs(diffHofProfiles(profile, norm));
    }
  }, [jsonText, profile]);
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url;
    const filename = `${(profile.name || "artist-character").replace(/\s+/g, "_").toLowerCase()}.json`;
    a.download = filename;
    a.click();
  ...
  const handleApply = useCallback((mode: "replace" | "merge") => {
    ...
    if (mode === "replace") {
      ...
      const merged: Partial<HallOfFameEntry> = { ...profile };
      (Object.keys(parsed) as (keyof HallOfFameEntry)[]).forEach((key) => {
  ...
  const btnPrimary = `${btnBase} bg-purple-500 text-white border-purple-400 hover:bg-purple-400 font-black`;
    ...
    <div className="flex flex-col gap-3 h-full">
      ...
      {validationState !== "idle" && (
        ...
        >
          ...
          <span className="font-bold">
            {validationState === "valid"
              ? "Artist / Hall of Fame JSON schema validation passed"
      ...
      <div className="relative flex-1 min-h-0">
        <textarea
          ...
          placeholder={`Paste HallOfFameEntry JSON here...\n\nExample:\n{\n  "name": "IU",\n  "type": "singer",\n  "status": "GOAT Status",\n  "knownFor": ["Hotel Del Luna", "Palette"],\n  "nationality": "South Korea"\n}`}
        />
