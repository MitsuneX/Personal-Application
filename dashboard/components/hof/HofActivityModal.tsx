    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 overflow-hidden">
        ...
        <motion.div
          ...
          className="relative w-full max-w-3xl max-h-[85vh] rounded-3xl flex flex-col overflow-hidden shadow-2xl z-10 border"
          style={{
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl md:text-2xl font-black font-mono tracking-tight theme-text-primary">
            ...
            <button
              onClick={onClose}
              className="w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-lg cursor-pointer transition-all hover:scale-105 active:scale-95 border"
              style={{
          ...
          <div className="p-4 md:p-5 border-b space-y-3 shrink-0" style={{ borderColor: isCyber ? "rgba(255,255,255,0.08)" : "#E2E8F0" }}>
            {/* Search Input */}
            <div className="relative">
              ...
              {searchQuery && (
                <button
                  ...
                  className="absolute right-3 top-2.5 text-xs font-mono opacity-60 hover:opacity-100"
                >
                  Clear
                </button>
              )}
                  ...
                  style={{
                    ...
                    borderColor:
                      selectedType === tf.id
                        ...
                        : "transparent",
                    borderWidth: isCyber ? "1px" : "2px",
                    boxShadow: selectedType === tf.id && !isCyber ? "2px 2px 0 #000" : "none",
                  }}
                >
                  <span>{tf.icon}</span>
                  <span>{tf.label}</span>
          ...
          {/* Activity List */}
          <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3 font-mono custom-scrollbar">
            {displayedActivities.length === 0 ? (
              <div className="py-12 text-center text-sm font-mono theme-text-muted">
              ...
              displayedActivities.map((act) => (
                <div
                  key={act.id}
                  className="p-4 rounded-2xl border text-xs flex items-center justify-between gap-4 transition-all"
                  style={{
                ...
                >
                  <div className="flex items-center gap-3">
                    <span className="text-2xl p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 shrink-0">
                      {act.type === "champion"
                        ? "👑"
                        : act.type === "goat"
                        ? "💎"
                    ...
                    <div>
                      <strong className="theme-text-primary text-sm font-black block">
                        {act.title}
                      </strong>
                      <span className="theme-text-muted text-xs leading-relaxed block mt-0.5">
                        {act.description}
                      </span>
                    </div>
                  </div>
