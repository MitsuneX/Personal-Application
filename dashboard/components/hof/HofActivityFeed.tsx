...
import { HofActivityModal } from "./HofActivityModal";
...
export function HofActivityFeed({ activityFeed, isCyber }: HofActivityFeedProps) {
  ...
  const topFiveFeed = activityFeed.slice(0, 5);
  ...
  return (
    <>
        ...
        {activityFeed.length > 0 && (
          <div className="pt-2 text-right">
            <button
              ...
              className="px-5 py-2 rounded-2xl text-xs font-mono font-bold transition-all cursor-pointer inline-flex items-center gap-2 border hover:scale-105 active:scale-95"
              style={{
                ...
                borderWidth: isCyber ? "1px" : "2px",
                boxShadow: isCyber ? "0 0 12px rgba(0,245,255,0.2)" : "3px 3px 0 #000000",
              }}
            >
              <span>View More ({activityFeed.length} Events)</span>
              <span>→</span>
            </button>
          </div>
        )}
      ...
      {/* Centered Activity History Modal */}
      <HofActivityModal
        isOpen={isModalOpen}
