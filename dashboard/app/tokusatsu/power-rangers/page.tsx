...
export default function PowerRangersPage() {
  ...
  const handleDelete = useCallback((id: string, name: string) => {
    ...
    confirm({
      title: "Remove Power Ranger Hero",
      message: `Are you sure you want to remove "${name}" from Power Rangers roster?`,
      confirmText: "Remove Ranger",
      variant: "danger",
      itemPreview: {
        title: name,
        subtitle: `Power Ranger · ${entry?.nationality || "Global"}`,
        description: Array.isArray(entry?.knownFor) ? entry.knownFor.join(", ") : entry?.knownFor,
        imageUrl: entry?.imageUrl,
        icon: "⚡",
        category: "Power Rangers",
      },
      successToast: `✓ "${name}" removed from Power Rangers roster.`,
      onConfirm: async () => {
        await deleteHof(id);
      },
    });
