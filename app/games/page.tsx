...
function GameCard({ game, isCyber, index }: { game: GameEntry; isCyber: boolean; index: number }) {
  ...
  return (
    <motion.div variants={cardVariants} custom={index}>
      <motion.div
        className="rounded-xl p-5 h-full flex flex-col gap-4 relative overflow-hidden"
        style={{
      ...
      >
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="font-black text-sm leading-tight" style={{ color: isCyber ? "#E0E8FF" : "#1A1A1A" }}>{game.game}</h3>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          {game.rank && (
            <div className="flex flex-col gap-0.5 px-3 py-2 rounded-lg" style={{ background: isCyber ? `${tier.cyberColor}10` : `${tier.brutColor}10`, border: isCyber ? `1px solid ${tier.cyberColor}30` : `1.5px solid ${tier.brutColor}` }}>
              <span className="text-xs uppercase tracking-widest font-bold" style={{ color: isCyber ? `${tier.cyberColor}80` : `${tier.brutColor}80` }}>Rank</span>
              <motion.span className="font-black text-base"
          ...
          <div className="flex flex-col gap-0.5 px-3 py-2 rounded-lg" style={{ background: isCyber ? `${accent}08` : `${accent}12`, border: isCyber ? `1px solid ${accent}25` : `1.5px solid ${accent}` }}>
            <span className="text-xs uppercase tracking-widest font-bold" style={{ color: isCyber ? `${accent}80` : `${accent}` }}>Main</span>
            <span className="font-black text-sm" style={{ color: isCyber ? "#E0E8FF" : "#1A1A1A" }}>{game.mainCharacter}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mt-auto">
          ...
          {game.handle && (
            <span className="text-xs font-mono" style={{ color: isCyber ? "rgba(255,255,255,0.3)" : "rgba(0,0,0,0.35)" }}>{game.handle}</span>
          )}
        </div>
      </motion.div>
    </motion.div>
    ...
    <AppShell>
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <p className="text-sm opacity-60" style={{ color: isCyber ? "#E0E8FF" : "#1A1A1A" }}>
      ...
      <div className="mb-8">
        ...
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4" variants={gridContainerVariants} initial="hidden" animate="visible">
          {gacha.map((g, i) => <GameCard key={g.id} game={g} isCyber={isCyber} index={i} />)}
        </motion.div>
      </div>
      ...
      <div>
        ...
        <motion.div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4" variants={gridContainerVariants} initial="hidden" animate="visible">
          {other.map((g, i) => <GameCard key={g.id} game={g} isCyber={isCyber} index={i} />)}
        </motion.div>
      </div>
    </AppShell>
  );
